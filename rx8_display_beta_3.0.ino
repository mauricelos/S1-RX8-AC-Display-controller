...
// acAmpTX
byte acAmpDataOut[5];
// Default data for the A\C amplifier TX; basicly it means I am here and no buttons have been pressed.
byte acAmpDataDefault[5] = {0x04, 0x80, 0x80, 0x80, 0xFC};

unsigned long acAmpTxBetweenTime;
unsigned long acAmpTxWaitTime = 18;  // Control the cycle time otherwise we have a continuous data stream allowing for
                                     // processing time this equates to 6-7 ms (most of the time).
...
// iconArray for display icons 90-9D, first 5 LSB, calculated via addition. iconArray[0] = current, iconArray[1] = LCD
...
void setup()
{
    ...
    // rtc.adjust(DateTime(F(__DATE__), F(__TIME__))); // your PC's time. ***** You may need to comment out after 1
    // upload to set the time - see wiki*******
...
void loop()
{
    ...
    if (!confMode)
    {
        ...
        if (millis() - acAmpCycleTime >= 200)  // we force the state of acAmpRxChanged if there has been no recent data
                                               // update, this way we minimise unnecessary processing but at acAmp power
                                               // off our icons clear more\less instantly (to the human eye).
        {
            acAmpRxChanged = true;
            // Enabling these would give us a timed refresh regardless of change, this may be needed if we encounter
            // issues\interference acAmpMessageToProcess = true; acAmpCycleTime = millis();
        }
...
void acAmpRecievedStatus()  // Based on the first byte of our inbound message we have 3 main data states from AC
...
void acAmpGetSerialData()
{
    ...
    {  // acAmpOn = true;
        ...
        if (acAmpNewByte == 0x0F || acAmpNewByte == 0x0D ||
            acAmpNewByte == 0x0E)  // We don't know the last byte but we know the first byte will be one of these
        {
            acAmpIndex = 0;
        ...
        else
        {
            ...
            if (acAmpNewByte != acAmpRxData[acAmpIndex])
            {
                ...
                if (acAmpIndex == 5)  // we know the length of the data transfer. If we knew how to calculate the
                                      // checksum we could validate received data but for now we assume it's correct.
                {
    ...
    {
        if (millis() - acAmpReceivedTime > 30 &&
            !acAmpDisplayClear)  // If there is no data we assume the A\C amplifier is off and we run 1 cycle to clear
                                 // the display. This is easier than having to monitor assorted vehicle power states.
        {
            acAmpOn = false;
            acAmpMessageToProcess = true;
...
void iconCalculateMode(byte _modeSet)  // Mode 1 = feet, 2 = feet\demist, 3 = face, 4 = face\feet
{
    switch (_modeSet)
    {
        case 1:
            ...
            break;
        case 2:
            if (!bitRead(acAmpRxData[4], 2) &&
                bitRead(acAmpRxData[4], 6))  // feet/demist has dependancies on addition demist related bytes
            {
                iconArray[0][12] += 0x02;
                iconArray[0][12] += 0x04;
            }
...
// Default values for iconArray [0][1] vertical divider, [0][3] HH:MM divider, [0][11] vertical divider, iconArray[1][x]
// = hex mapping for display address.
...
{
    hourCalculate(
        currentHour);  // we include this here as must be reclaculated everytime we update regardless of change.
...
void calculateTime()
{
    if (millis() - nowTimeCycle >= 10000 ||
        confMode)  // 'DateTime now' takes over 1 milliSecond to return a result, so we don't run it every cycle.
    {
        DateTime now = rtc.now();
        nowTimeCycle = millis();
        nowHour = now.hour();
    ...
    // If the hour or minute are continually updating the other icons flicker\vanish as they are not set constantly, so
    // we flag a minute change to manage display updates.
...
void acAmpProcessTemp()
{
    ...
    for (byte i = 0; i <= 2; i++)
    {
        ...
        if (minuteChange || acAmpMessageToProcess)  // as the temp is via seven segment, we need to make sure we update
                                                    // if the time changes.
        {
...
void checkTempRotation()
{
    ...
    {
        if ((aTempNew && !aTempOld && !bTempNew && !bTempOld) ||
            (!aTempNew && aTempOld && bTempNew && bTempOld))  // Right movement
        {
            if (!confMode)
            {
        ...
        else if ((aTempNew && aTempOld && !bTempNew && bTempOld) ||
                 (!aTempNew && !aTempOld && bTempNew && !bTempOld))  // Left movement
        {
            if (!confMode)
            {
...
void checkPushedButton()
{
    ...
    if (pushedButtonCurrent == 0 &&
        buttonCycle)  // Long button press is single shot, so must be reset by removing finger.
    {
        buttonCycle = false;
    }

    if ((!longPushButton && pushedButtonCurrent == pushedButtonOld) && pushedButtonCurrent &&
        (millis() - buttonPushed >= buttonDebounce))  // index the button count
    {
        buttonCount++;
        buttonPushed = millis();
    }
...
void confMenu()
{
    ...
    switch (menuIndex)  // Also set subMenu and index options
    {
        case 1:
            ...
            if (shortPushButton == 1)
                acAmpCentigrade = (!acAmpCentigrade) ? true : false;
            iconArray[0][6] = (acAmpCentigrade) ? iconArray[0][6] += 0x10 : iconArray[0][6] += 0x08;
            break;
        case 2:
